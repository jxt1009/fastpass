...
- 2026-06-12 (track C implementer): E-8 dropped — `min()`/`max()` on `[CLLocationDegrees]` return `Element?` (optional) in Swift 6 even after `guard !routeCoordinates.isEmpty`. Must keep the `!`.
- 2026-06-12 (track C implementer): E-10 approach changed from custom `Drive.==` to version-guarded `@retroactive` on `CLLocationCoordinate2D`, because `DriveStatsSnapshot` and `ZeroToSixtyAttemptDisplay` in RecordingActor.swift (outside track C's file list) depend on `CLLocationCoordinate2D: Equatable`.

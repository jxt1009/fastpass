...
- 2026-06-12: For `routePointSpeed`, refactored to take `latestSample` as an explicit parameter (`latestSample: SpeedSample?`) so the method can be `nonisolated`. Caller in `processLocation` passes `latestSpeedSample` from the main actor context. No behavioral change.
- 2026-06-12: For `processHeading`, marked `nonisolated` because it already handles the `@MainActor` hop manually via `MainActor.run { self.leftTurns += ... }` internally.
- 2026-06-12: For AuthManager keychain accessors, extracted a `private struct KeychainStore: Sendable` with static `save/load/delete` methods. Token accessors (`getToken`, `saveToken`, `getRefreshToken`, `saveRefreshToken`) are `nonisolated`. `clearTokens` stays on `@MainActor` because it sets `isAuthenticated`, a `@Published` var.
- 2026-06-12: Sendable conformance added to `Drive`, `SpeedSample`, `ZeroToSixtyAttempt`, and `RunningSpeedStats`. `RecordingActor` is already an actor (implicitly Sendable). `RouteSerializationSnapshot` already conforms to Sendable.
- 2026-06-12: For E-15, the `PinningURLSessionDelegate` is stored as `private let pinningDelegate` on `APIService` so it stays alive for the lifetime of the service. The URLSession is initialized with `configuration: delegate: delegateQueue:` instead of `URLSession.shared`.
- 2026-06-12: `startRecordingForTest` / `stopRecordingForTest` don't exist in the codebase. The concurrency test (`DriveManagerConcurrencyTests`) manually sets `isRecording = true` and assigns a minimal `currentDrive` instead.
- 2026-06-12: `@MainActor` on `DriveManager` caused cascading test errors in `PersonalBestsTests`, `CurrentDriveDedupeTests` (DriveManager init + property access) and `RecordingActorHeadingTests` (`resetHeading()` needs `await`). Fixed by annotating test classes with `@MainActor` and adding `await` to actor calls.
- 2026-06-12: Swift 6 strict-concurrency build (`-strict-concurrency=complete`) produced only 2 warnings (well under the 5-warning threshold). No blocking changes needed.

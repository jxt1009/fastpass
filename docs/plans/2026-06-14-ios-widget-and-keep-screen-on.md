...
- _2026-06-14_ — Executed Task 4 (struct changes for `phase` / `elapsedSeconds`) before Task 3 (protocol + async coordinator). The plan lists them in opposite order, but Task 3's coordinator implementation constructs `DriveActivityState(phase: .ended, ...)` literals that need the new field. Doing Task 4 first keeps each commit buildable; the backward-compat decode test in `LiveActivityCoordinationTests` is still landed in the same Task 4 commit. — _Rationale: Preserves a green-build invariant per commit; no semantic change vs the plan.
- _2026-06-14_ — Pre-existing test failures observed when running the full suite. The following tests fail identically on `main` (verified by running them on the main worktree against `main` HEAD): `DriveDeleteTests.testDriveManager_deleteDrive_removesFromArray`, `…_treats404AsSuccess`, `DriveManagerErrorSurfaceTests.testStopRecording_setsLastError_whenCreateDriveThrows`, `…_whenCreateDriveTimesOut`, `CarHeroPhotoEditorSheetTests.testCarDetailView_heroHasPencilCircleFillOverlay`, `ProfileRedesignTests.testProfileView_AchievementsStripAboveGarage`, `…_HasViewGarageLink`, `CarDetailGaugeProgressWiringTests.testCarDetailView_pbGauges_GatesProgressOnDataPresence`, `…_PassesProgressFromHelper`, `CurrentDriveDedupeTests.test_noFiringWhenDriveUnchanged`. None of these tests are touched by this PR. — _Rationale: Final report flags these as "pre-existing" so the reviewer doesn't conflate them with the PR's scope.
- _2026-06-15_ — Final-review fixup commit (`91d3a10`) restored the `ScreenWakeController` wiring in `FastTrackApp.swift` that was accidentally dropped during Task 7's "git checkout main -- ios/ + git stash pop" recovery. The Task 2 commit (`909d3d7`) had correctly added the `@StateObject screenWake`, the four `.onChange`/`.onAppear` hooks, and the env-object plumbing for both `#if DEBUG` and `#else` paths; the orphan-sweep commit (`80cbd7b`) incidentally reverted all of it, leaving `ScreenWakeController` created but never instantiated. Net effect of the fixup: the contract from Task 2 is restored verbatim. — _Rationale: Without this, `isIdleTimerDisabled` would have been permanently false — keep-screen-on toggle a no-op, the user's screen would sleep mid-drive — i.e. Bug 2 would have shipped worse than it started.
- _2026-06-15_ — Two additional tests failed in the final full-suite run that were not on the implementer's pre-existing list: `DriveManagerHeadingTests.test_headingTotalsAccumulate` and `ForceUnwrapAuditTests.test_DriveEquatable_zeroToSixtyAttemptsDiffers`. Static analysis: neither touches anything this PR changes — the heading test uses `RecordingActor.shared` global state (flaky, shows the retry pattern in the test runner) and the audit test compares `Drive` equality on `zeroToSixtyAttempts` which is unrelated to widget/idle-timer code. Treated as pre-existing (or pre-existing flakes) on the same basis as the implementer's other ten. — _Rationale: The PR's diff cannot plausibly affect them; out of scope.

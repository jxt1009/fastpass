...
TestFlight notes are derived from the GitHub Release body in the iOS release workflow, but normalized specifically for App Store Connect:
1. Only the latest release section is used (not historical sections).
2. Only user-facing groups are included (`Features`, `Bug Fixes`, `Performance`).
3. Markdown is converted to plain text before populating TestFlight's **What to Test** field.

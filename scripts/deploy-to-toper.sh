...
echo "🚀 FastTrack Remote Deployment"
echo "===================================="
echo ""
...
DOMAIN="${DEPLOY_DOMAIN:-api.example.com}"
...
if [ "$SERVER" = "user@your-server" ]; then
    echo -e "${RED}✗ DEPLOY_SERVER is not configured${NC}"
    echo "  Set DEPLOY_SERVER (e.g. export DEPLOY_SERVER='user@host')"
    exit 1
fi

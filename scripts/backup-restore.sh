...
case "$1" in
    backup)
        ...
        kubectl exec -n $NAMESPACE $POSTGRES_POD -- sh -c "
            pg_dump -U fasttrack -d fasttrack_production > ${BACKUP_DIR}/${BACKUP_FILE}
    ...
    restore)
        ...
        if [[ $BACKUP_FILE == *.gz ]]; then
            kubectl exec -n $NAMESPACE $POSTGRES_POD -- sh -c "
                gunzip -c ${BACKUP_DIR}/${BACKUP_FILE} | psql -U fasttrack -d fasttrack_production
            "
        else
            kubectl exec -n $NAMESPACE $POSTGRES_POD -- sh -c "
                psql -U fasttrack -d fasttrack_production < ${BACKUP_DIR}/${BACKUP_FILE}
            "
        fi
    ...
    test)
        ...
        kubectl exec -n $NAMESPACE $POSTGRES_POD -- psql -U fasttrack -d fasttrack_production -c "SELECT 'Connection successful!' as status;"
        echo ""
        echo "Database size:"
        kubectl exec -n $NAMESPACE $POSTGRES_POD -- psql -U fasttrack -d fasttrack_production -c "
            SELECT
                pg_size_pretty(pg_database_size('fasttrack_production')) as size,
